        ...
        for m in range(Number_of_Models):
            for state in States:
                for action in range(Params["Num_Amylase_States"]):
                    ...
                    if Temp_L.__len__() != 0:
                        for k in range(Temp_L.__len__()):
                            
                            UD[m][(state, action)]+=alpha*(Temp_L[k]-UD[m][(state, action)])
        for m in range(Number_of_Models):
            Temp_Pol = {}
            for state in States:
...
def Generate_Episodes_With_State(dFBA, States, Params, Init_C, Models, Mapping_Dict, t_span=[0, 100], dt=0.1, Num_Episodes=2, Gamma=1):
    ...
    for k in range(Num_Episodes):
        ...
        #                           ].append(f_vals[st][m])
        for m in range(Models.__len__()):
            Returns_Totall[m][(States[0], Actions[0][m])
                                   ].append(np.sum(f_vals,axis=0)[m])
