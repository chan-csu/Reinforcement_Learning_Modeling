...
tm1= mc.Model(reactions=tm.Toy_Model_NE_1.reactions,metabolites=tm.Toy_Model_NE_1.metabolites,objective="Biomass_1")
tm2= mc.Model(reactions=tm.Toy_Model_NE_2.reactions,metabolites=tm.Toy_Model_NE_2.metabolites,objective="Biomass_2")


agent1=tmc.Agent("agent1",
				model=tm1,
				actor_network=tmc.NN,
				critic_network=tmc.NN,
				clip=0.1,
				lr_actor=0.0001,
				lr_critic=0.0001,
				grad_updates=1,
...
env=tmc.Environment(name="Toy-NECOM-host",
					agents=agents,
					dilution_rate=0.0001,
...
for batch in range(env.number_of_batches):
	batch_obs,batch_acts, batch_log_probs, batch_rtgs=tmc.rollout(env)
	for agent in env.agents:
