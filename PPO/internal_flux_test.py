...
import internal_flux_controller as tk
...
agent1=tk.Agent("agent1",
				...
				actions=[a for a in tm.Toy_Model_NE_1.reactions if a not in tm.Toy_Model_NE_1.exchanges ],
				gamma=1,
				)
...
agent2=tk.Agent("agent2",
				...
				optimizer_critic=torch.optim.Adam,       
				observables=['agent1','agent2','S',"A","B"],
				actions=[a for a in tm.Toy_Model_NE_2.reactions if a not in tm.Toy_Model_NE_2.exchanges ],
				gamma=1
)

agents=[agent1,agent2]

env=tk.Environment(name="Toy-NECOM-internal",
					agents=agents,
					dilution_rate=0.0001,
