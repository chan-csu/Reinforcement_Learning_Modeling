...
def main(Models: list = [ToyModel.copy(), ToyModel.copy()], max_time: int = 100, Dil_Rate: float = 0.1, alpha: float = 0.01, Starting_Q: str = "FBA",Episode_Per_Run=5):
    ...
    Params["State_Inds"]=list(range(len(Models)))
    ...
    Outer_Counter = 0
    # Q initalization ###------------------------------------------------------------
    for _ in range(Episode_Per_Run):
...
def Generate_Episodes_With_State(dFBA, Params, Init_C, Models, Mapping_Dict, t_span=[0, 100], dt=0.1):
    ...
    Init_C[Params["Agents_Index"]]=random.uniform(Params["Agent_Max_C"]*0.5, Params["Agent_Max_C"]*1.5)
